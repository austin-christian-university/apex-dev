...
import { CalendarDays, Trophy, TrendingUp, ChevronDown, ChevronRight } from "lucide-react"
...
export default function CompanyStandingPage() {
  const { user } = useAuth()
  ...
  const [actionRequiredExpanded, setActionRequiredExpanded] = useState(false)
  ...
  return (
    <div className="px-4 py-6 space-y-6 max-w-md mx-auto">
      ...
      {urgentEvents && urgentEvents.length > 0 && (
        <Collapsible open={actionRequiredExpanded} onOpenChange={setActionRequiredExpanded}>
          <Card>
            <CollapsibleTrigger asChild>
              <CardContent className="p-4 cursor-pointer">
                <div className="flex items-center justify-between">
                  ...
                  {actionRequiredExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                </div>
              </CardContent>
            </CollapsibleTrigger>
            <CollapsibleContent>
